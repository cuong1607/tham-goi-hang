...
type MissingItem = { pattern: string; caption: string };

// ── Color tokens (blue theme for Cừu) ────────────────────────
const C = {
  primary:       "#1d4ed8",
  primaryLight:  "#eff6ff",
...
const SIZE_DISPLAY: Record<string, string> = {
  "50x70":"50cmx70cm","60x60":"60cmx60cm","60x90":"60cmx90cm",
  "80x120":"80cmx120cm","50x120":"50cmx120cm","80x200":"80cmx200cm",
  "100x100":"100cmx100cm","120x120":"120cmx120cm","120x160":"120cmx160cm",
  "140x200":"140cmx200cm","160x160":"160cmx160cm","160x230":"160cmx230cm",
  "200x300":"200cmx300cm",
};
...
function clientParse(text: string): ParsedItem[] {
  ...
  const rawItems: Array<{pattern:string;nPat:string;size:string;qty:number}> = [];

  for (const line of lines) {
    const ci = line.indexOf(":");
    if (ci === -1) continue; // Cừu không có group header
    const size = normSize(line.slice(0, ci).trim());
    for (const raw of splitPats(line.slice(ci+1))) {
      const clean = cleanPat(raw);
      const nPat = clean.toLowerCase().replace(/\s+/g, " ").trim();
      rawItems.push({ pattern: clean, nPat, size, qty: extractQty(raw) });
    }
  }

  // Aggregate by nPat
  const map = new Map<string, {pattern:string;nPat:string;sizeMap:Map<string,number>}>();
  for (const it of rawItems) {
    let e = map.get(it.nPat);
    if (!e) { e = {pattern:it.pattern,nPat:it.nPat,sizeMap:new Map()}; map.set(it.nPat, e); }
...
// ── Main Page ─────────────────────────────────────────────────
export default function CuuImageGeneratorPage() {
  ...
  const handleParse = useCallback(() => {
    ...
    try {
      ...
      if (items.length === 0) { setError("Không parse được text. Kiểm tra format: size: mã, mã(qty)"); return; }
      setParsed(items);
      setResults(null);
      setError("");
      setZipBase64(null);
  ...
  const handleDownloadZip = useCallback(() => {
    ...
    a.href = url;
    a.download = `cuu-to-goi-hang-${Date.now()}.zip`;
    a.click();
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/cuu-to-images/generate", {
        method: "POST",
  ...
  const EXAMPLE = `80cmx120cm: LC_1%, LC_Cầu Vồng
...
1m4x2m: LC_1%`;
        ...
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <div>
                <h1 style={{ fontSize: 22, fontWeight: 800, color: C.text, margin: 0 }}>
                  🐑 Sinh Ảnh Gọi Hàng Thảm Cừu To
                </h1>
                <p style={{ color: C.muted, fontSize: 13, marginTop: 6, marginBottom: 0 }}>
                  1 mẫu = 1 ảnh · Gom theo pattern · Overlay size tự động · Export ZIP
                </p>
              </div>
            </div>
          </div>
          ...
          <div style={cardStyle()}>
            ...
            <div style={{
              ...
              <strong>Format:</strong> <code>size: mã, mã(qty)</code> &nbsp;|&nbsp; Ảnh mẫu: <code>public/assets/cuu-to/pattern.jpg</code>
            </div>
          </div>
                ...
                {activeTab === "gallery" && (
                  generatedItems.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "40px 0", color: C.muted }}>
                      ...
                      <span style={{ fontSize: 12 }}>Hãy đặt ảnh mẫu vào <code>public/assets/cuu-to/pattern.jpg</code></span>
                    </div>
                  ) : (
                    <div style={{
                      display: "grid",
                            ...
                            {missingImages.map((m, i) => (
                              <div key={i} style={{
                                ...
                                <code style={{ marginLeft: "auto", fontSize: 11, color: "#555" }}>
                                  public/assets/cuu-to/{m.pattern}.jpg
                                </code>
                              </div>
                            ))}
                          </>
  ...
  assets/
    cuu-to/
      LC_1%.jpg       ← tên file = tên mã màu
      LC_100%.jpg
      LC_Cầu Vồng.jpg`}</pre>
              <div style={{ fontSize: 12, color: C.muted, marginTop: 10 }}>
                Hỗ trợ định dạng: jpg, jpeg, png, webp · Tên file phải khớp chính xác tên mã màu
              </div>
            </div>
          )}
